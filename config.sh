...
# purge  variables and functions on exit
cleanup () {
    unset system
    unset tunnel_info
    unset -f get_tunnel_info
    unset -f Help
}
...
if [ "$system" = "prod" ]; then
    ...
    echo export ARCHIVE_DB_SECRET_NAME=hopProd-archive-ingest-db-password
    echo export ADMIN_DB_INSTANCE_NAME=prod-scimma-admin-postgres
    echo export ADMIN_DB_SECRET_NAME=prod-scimma-admin-db-password
    ...
    echo export ARCHIVE_TUNNEL_REMOTE_HOST=$(echo $tunnel_info | jq -r '.Endpoint.Address')
    echo export ARCHIVE_TUNNEL_REMOTE_PORT=$(echo $tunnel_info | jq -r '.Endpoint.Port')
    ...
    echo export ADMIN_TUNNEL_REMOTE_HOST=$(echo $tunnel_info | jq -r '.Endpoint.Address')
    echo export ADMIN_TUNNEL_REMOTE_PORT=$(echo $tunnel_info | jq -r '.Endpoint.Port')
    ...
    echo export ARCHIVE_DB_SECRET_NAME=hopDevel-archive-ingest-db-password
    echo export ADMIN_DB_INSTANCE_NAME=scimma-admin-postgres
    echo export ADMIN_DB_SECRET_NAME=scimma-admin-db-password
    ...
    echo "environment configured local pastgres databases"  >&2
    echo export ARCHIVE_DB__NAME=postgres
    echo export ARCHIVE_DB__USER=postgres
...
echo export OIDC_RP_CLIENT_ID=dog
...
echo export OIDC_RP_CLIENT_SECRET_NAME=dog
